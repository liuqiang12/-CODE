...
public class StringUtil {
	 ...
	 * @throws Exception
	 */
	public static Map<String, String> getNetMaskByShortType(String shortType) throws Exception {
	...
	public static String[][] getStringMapIn2D(String sourceString) {
		...
		for (int i = 0; i < rows; i++) {
			columns = Math.max(columns, trimRedundancyWhiteSpaceInStr(rowString[0]).split(" ").length);
		}
		...
		for (int i = 0; i < rows; i++) {// 对stringMap 进行填充
			temp = trimRedundancyWhiteSpaceInStr(rowString[i].trim()).split(" ");
	 ...
	 * @return
	 */
	public static String getTheCellStringIn2D(String sourceString, int row, int column) {
	 ...
	 * @return
	 */
	public static String[] getTheColumnStringIn2D(String sourceString, int column) {
	...
	/**
	 * 根据short type 得到ip 与掩码 再计算该子网内的所有可用ip
	 * 
	 * @param shortType
	 * @return
	 * @throws Exception
	 */
	public static String[] getIPsV4ByIpShortType(String shortType) throws Exception {
		Map<String, String> map = getNetMaskByShortType(shortType);
		if (map == null || map.size() == 0)
			return null;
		return getIPsV4InSubnet(map.get("ip"), map.get("mask"));
	}
	 ...
	 * @return
	 */
	public static String[] getIPsV4Between(String startIp, String endIp, boolean contained) {
		
		return IPUtil._getIPArraysBetween(startIp, endIp);
	 ...
	 * @return
	 */
	public static String[] _getIPsV4Between(String startIp, String endIp, boolean contained) {
		
		String[] start = startIp.split("\\.");
		String[] end = endIp.split("\\.");
		if (start.length != end.length)
			return null;
		int startNum = Math.min(Integer.valueOf((Integer.parseInt(start[start.length - 1]))), Integer
				.valueOf(end[end.length - 1]));
		int endNum = Math.max(Integer.valueOf((Integer.parseInt(start[start.length - 1]))), Integer.valueOf(end[end.length - 1]));
		...
		String[] ips;
		if (contained) {
			ips = new String[endNum - startNum + 1];
		}
		else {
			ips = new String[endNum - startNum - 1];
		}
		...
		for (int i = 0; i < length; i++) {
			if (contained) {
				...
				lastNum = startNum + i + 1;
			}
			
			ips[i] = preffix + lastNum;
	 ...
	 * @throws Exception
	 */
	public static String[] _getIPsV4InSubnet(String ip, String netMask) throws Exception {
		if (!isIpFormat(ip))
		...
		int baseNum = Integer.parseInt(ip.substring(ip.lastIndexOf(".") + 1).trim()) & Integer.parseInt(netMaskElements[index]);
	 ...
	 * @throws Exception
	 */
	public static String[] getIPsV4InSubnet(String ip, String netMask) throws Exception {
		return IPUtil._getIPArrays(ip, netMask);
	...
	public static void main(String[] args) {
		...
		try {
			String[] s = getIPsV4ByIpShortType("192.168.40.3/32");
			for (String ip : s) {
				System.out.println(ip);
			}
		} catch (Exception e) {
		...
		// s =getMaskBinaryStr(8,0);
		// s =getNetNOBy("192.168.70.1",8);
		// System.out.println(s);
		// String[] s1 =getIPsV4Between("192.168.50.4","192.168.50.11",true);
	 ...
	 * @return
	 */
	public static String parseStr(String source,String charsetName) {
		...
		if (source != null && !"".equals(source.trim())) {
			...
			for (int i = 0; i < arr.length; i++) {
				...
				in = Integer.parseInt(arr[i], 16);
				byArr[i] = (byte) in;
				// System.out.println(arr[i] + ":" + in + ":" + byArr[i]);
			}
			String dest = null;
			try {
				dest = new String(byArr,charsetName);
