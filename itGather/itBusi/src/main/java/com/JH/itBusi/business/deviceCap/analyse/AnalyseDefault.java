	...
	public int doMemAnalyse(ArrayList<Integer> mem) {
		...
		if (hsMemoryUtilization == null || hsMemoryUtilization.size() == 0) {
			...
			for (Entry<String, Object> ent : hsMemoryUtilization.entrySet()) {
				// 原为直接转换为int，中兴 5系列的OID拉回来的值为 double 类型，需要先转换为double类型，再通过intValue 得到 int值。（2015-04-20 liyi）
				Double doubleValue =Double.parseDouble(hsMemoryUtilization.get(ent.getKey()).toString());;
				memoryprocess = doubleValue.intValue();
			}
