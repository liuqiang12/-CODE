...
<div class="easyui-layout" fit="true" style="overflow-x:hidden;background-color: rgb(73,73,73)">
    ...
    <div data-options="region:'center'" title="用电信息">
        <table id="dg" class="easyui-datagrid"
            ...
            <tr>
                <th data-options="field:'read1',width:100,formatter:function(value,row,index){
                   return value.toFixed(2)

                }">支路1</th>
                <th data-options="field:'read2',width:100,formatter:function(value,row,index){
                   return value.toFixed(2)
                ...
                <th data-options="field:'diff1',width:100,formatter:function(value,row,index){
                   return value.toFixed(2)

                }">支路1</th>
                <th data-options="field:'diff2',width:100,formatter:function(value,row,index){
                   return value.toFixed(2)
                }">支路2</th>
...
//        $("#datetime").datebox({value: date.add("d", -1).Format("yyyy-MM-dd")})
        var tree = $("#rtree").rtree({
            ...
            , onClick: function (iteam, treeId, treeNode) {
                if (treeNode.id.indexOf("room_") > -1) {
                    buildid = treeNode.id.split("room_")[1];
                    getData();
                }
            }
        });
        var timeer = setInterval(function () {
                    if (typeof tree.rtree != 'undefined' && tree.rtree != null) {
                        ...
                        var nodes = tree.rtree.getNodesByParamFuzzy("id", "room_", null);
    ...
    function getData() {
        if (buildid && buildid > 0) {
            ...
            $.post(contextPath + "/pe/racklist", {roomid: buildid, startTime: times[0], endTime: times[1]}, function (data) {
                layer.close(index);
