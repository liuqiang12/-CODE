...
<head>
    ...
    <STYLE>
        ...
        .rows--mini .row__seat--no,
        .rows--mini .row__seat--no:hover {
            background:rgba(72, 72, 78, 0.5)
        }
...
<div region="south" split="false" border="false" class="titleTool" style="height: 50px">
    <ul class="legend">
        <li style="font-size:20px;display: inline-block;">电量利用率:</li>
        <li class="legend__item legend__item--canuse"><20%</li>
        <li class="legend__item legend__item--preused">>20%</li>
        <li class="legend__item legend__item--free">>40%</li>
        <li class="legend__item legend__item--runing">>60%</li>
        <li class="legend__item legend__item--stop">>80%</li>
        <li class="legend__item legend__item--other">>其他</li>
...
<div region="center" split="true" title="" class="history" style="position: relative;overflow: auto">
    <div style="width: 900px;height: 540px;">
        ...
        <div class="rows rows--mini" id="rowsdiv">
            <%-- 创建底层格子 --%>
            <mycell v-for="trow in celldata" :data="trow" :key="trow.r*trow.c"  style="position: absolute;z-index: 1;width: 69px;height: 69px;background: rgba(72, 72, 78, 0.5);opacity: 1.5;
                   margin: 0 0 1px 1px;border-radius: 1px;">
            </mycell>
            <%-- 创建机架 --%>
            <rackcell v-for="trow in rackata" :data="trow" :key="trow.r*trow.c"
                      style="position: absolute;z-index: 3;width: 65px;height: 65px;cursor: pointer">
            </rackcell>
            <%-- 创建模块 --%>
...
<script type="text/x-template" id="rackcelltmpl">
    <div v-else :data-mytooltip="data.data.name" :data-tipso="data.data.name"
        ...
        <div v-if="selected==false" style="color: white;font-size: 11px;margin-left: 2px;word-wrap: break-word;">
            <span tyle="margin-left: 2px;word-wrap: break-word;">{{namestr}}</span>
            <div v-if="data.data.businesstypeId=='equipment' ||data.data.businesstypeId=='cabinet'">{{data.data.power}}</div>
            <div v-if="data.data.businesstypeId=='equipment' ||data.data.businesstypeId=='cabinet'">{{data.data.powerread}}</div>
        </div>
        <div v-if="selected" class="selectedSty">
            <span tyle="margin-left: 2px;word-wrap: break-word;">{{namestr}}</span>
            <div v-if="data.data.businesstypeId=='equipment' ||data.data.businesstypeId=='cabinet'">{{data.data.power}}</div>
            <div v-if="data.data.businesstypeId=='equipment' ||data.data.businesstypeId=='cabinet'">{{data.data.powerread}}</div>
        </div>
...
<script src="<%=request.getContextPath() %>/js/pe/js/roomlayoutview.js?version=13" type="text/javascript"></script>
</html>