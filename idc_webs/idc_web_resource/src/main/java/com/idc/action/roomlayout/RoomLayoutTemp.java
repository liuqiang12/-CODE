...
public class RoomLayoutTemp {
    ...
    public void updateRackXAndY(List<Row1> rows, List<IdcRack> list) {
        ...
        for (IdcRack idcRack : list) {
            ...
            if (name.indexOf("PX") > -1) {//配线柜
                ...
                rowMax = rowMax>row?rowMax:row;
                cellMax = cellMax>cell+2?cellMax:cell+2;
            }
        }
        //处理ODF架排列位置
        updateOdfRackXAndY(rows, odfList,rowMax);
        //处理PDF架排列位置
        updatePdfRackXAndY(rows, pdfList,cellMax);
    }

    //处理PDF架排列位置
    public void updatePdfRackXAndY(List<Row1> rows, List<IdcRack> list,int cellMax) {
        for (IdcRack idcRack : list) {
            String name = idcRack.getCodeView();
    ...
    //处理ODF架排列位置
    public void updateOdfRackXAndY(List<Row1> rows, List<IdcRack> list,int rowMax) {
        for (IdcRack idcRack : list) {
            String name = idcRack.getCodeView();//YD_SC_CD_XY_B04_207_ODF_A01_b/YD_SC_CD_XY_B04_207_ODF_A01
            String[] split = name.split("_");
            ...
            if (StringUtil.isNumeric(typelast)) {//判断倒数第3个是否是数字   若为数字   则表示为普通ODF柜  直接放在最后面  单独成行
                ...
                int row = c - 65;
                int cell = Integer.parseInt(last.substring(1));
                addRow(rows, rowMax+row+2, cell + 2, idcRack);
                ...
                for (int i = 0; i < rows.size(); i++) {
                    ...
                    for (int j = 0; j < rows.get(i).getCells().size(); j++) {
                        ...
                        if (param.getCodeView().contains("WL")) {
                            flag = true;
                            break;
                        }
                    }
                    if (flag) {//存在网络架    暂时不处理
                        ...
                        if (row == i) {
                            if (row1 == 0) {//_a
                                for (int v = 0; v < rows.get(i).getCells().size(); v++) {
                                    if (rows.get(i).getCells().get(v).cellnum >= 3+row) {
                                        rows.get(i).getCells().get(v).setCellnum(rows.get(i).getCells().get(v).cellnum + 1);
                                    }
                                }
                                addRow(rows, i, 3+row, idcRack);
                            } else {//_b
                                for (int v = 0; v < rows.get(i).getCells().size(); v++) {
