...
<div id="test" style="padding:10px" data-options="region:'center'">
    <div style="border: 1px solid black; margin-left:30px;float: left">
...
<script type="text/x-template" id="sigjiwei">
    <div v-on:click="show(obj,index,$event)">
        ...
        <%--<div style="float:left;left:30px;right:150px;width: 100%">&nbsp;</div>--%>
...
<script>
    ...
    function check(start, max) {
        ...
        for (var i = parseInt(start); i < parseInt(start) + parseInt(max); i++) {
            //先校验数据
            if (vuex_store.state.datas[i].key != '') {
                alert('当前机位数不够')
                return false;
            }
        }
        ...
        methods: {
            rtree: function () {
                mylayer.open({
                    ...
                    fixed: false, //不固定
                    maxmin: true,
                    content: contextPath + "/idclink/rack?roomId=${rack.roomid}" + "&ids=" + rackid + "&flag=view"
//                    contextPath+'/idclink/rack/'+rackid
                });
            },
            deleteeq: function () {
            ...
            add: function () {
                var selectNode = vuex_store.state.selectNode;
                if (vuex_store.state.datas[selectNode].key != '') {
                    alert('当前机位不是空闲')
                    return;
                }
