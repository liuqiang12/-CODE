...
function calcNWmask(mask) {
    ...
    if (tmpvar >= 8) {
        ...
        maskstr[0]=h_fillbitsfromleft(tmpvar)
        return maskstr;
    }
    if (tmpvar >= 8) {
        maskstr[1]=255
        ...
        maskstr[1]=h_fillbitsfromleft(tmpvar)
        return maskstr;
    }
    if (tmpvar >= 8) {
        maskstr[2]=255
        ...
        maskstr[2]=h_fillbitsfromleft(tmpvar)
        return maskstr;
    }
    maskstr[3] =h_fillbitsfromleft(tmpvar);
    return maskstr;
}
 ...
 * @param mask
 */
function calNBFL(subnetip,mask) {
    var rt = 0;
    var ips = subnetip.split(".");
    if(ips.length!=4){
        return ;
    }
    for(var i=0;i<ips.length;i++){
        var tmpvar = parseInt(ips[i], 10);
        if (isNaN(tmpvar) || tmpvar > 255 || tmpvar < 0) {
            return;
        }
    }
    var masks = calcNWmask(mask);
    if (masks.length!=4) {
        return ;
    }
    ...
    if (tmpvar == 31) {
        ...
        var begips = [],endips=[];
        for(var s=0;s<4;s++){
            begips.push(ips[s] & masks[s]);
            endips.push(ips[s] | (~masks[s] & 0xff));
        }
        $("#begip").textbox("setValue",begips.join("."))
        //$("input[name='begip']").val(begips.join("."))
        $("#endip").textbox("setValue",endips.join("."))
        //$("input[name='endip']").val(endips.join("."))
        return ;
    }
    if (tmpvar == 32) {
        //只有一个地址 有问题 不继续
