...
$(function () {
    ...
    $("#edit").click(function(){
        ...
        $.post(contextPath+"/device/getDeviceIsUpRack.do?deviceid="+rows[0].DEVICEID,function(result){
            flag = result.msg;
            var url = contextPath + "/device/deviceDetails.do?id=" + rows[0].DEVICEID + "&buttonType=update&deviceclass=2&flag=" + flag;
            var btn=[];
            //视图上下架
    ...
    $("#exportData").click(function () {
        ...
        window.open(contextPath + '/idchost/exportDeviceData?searchStr='+searchStr);
        // $.post(contextPath +'/idchost/exportDeviceData',{},function(result){
        //     alert(result.msg);
        // });
    });
    function getOptions() {
        var option = {
            frozenColumns: [[
                ...
                {field: 'NAME', title: '设备名称', width: 250}
            ...
            columns: [[
                ...
                {field: 'RNAME', title: '归属机架', width: 200,formatter:function(value,row,index){
                    if(value != null && value != ''){
                ...
                // {field: 'SERVERIPADDRESS', title: '管理IP地址', width: 120},
                ...
                {
                    field: 'CUSTOMERNAME', title: '客户名称', width: 200, formatter: function (value, row, index) {
                    if (value != null && value != '') {
            ...
            checkOnSelect: true,
            onClickRow: fun,
            url: contextPath + "/device/list.do?deviceclass=2",
            onDblClickRow: function (index, row) {
                var url = contextPath + "/device/deviceDetails.do?id=" + row.DEVICEID + "&buttonType=view&deviceclass=2";
...
function searchModels() {
    ...
    $('#dg').datagrid({
        ...
        queryParams: {name: name, deviceclass: 2, searchType:searchType,searchId:searchId}
    });
}
//导入
function importFileClick(){
    // $.ajaxFileUpload({
    //     type: "POST",
    //     url: contextPath + "/idchost/importDeviceData",
