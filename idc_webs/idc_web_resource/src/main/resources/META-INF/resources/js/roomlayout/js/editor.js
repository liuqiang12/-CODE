...
propertyPanel.prototype.loadTopology = function (backImg,templateId,topologyId,topoLevel) {
    ...
    $.ajax({
        url: context + '/roomlayout/getNodes/'+roomid,
        async: false,
        type: "GET",
        dataType: "json",
        error: function () {
        ...
        success: function (data) {
            $(data).each(function(i,iteam){
                ...
                var newn = editor.utils.cloneObj(iteam);
                var newNode =  editor.addNode(newn);
            });
        }
    });
    };
...
propertyPanel.prototype.deleteAllNodes = function(templateId){
    ...
    var self = this;
    jConfirm("确定要清空机房布局吗?","清空机房布局",function(ok){
        if(ok){
...
networkTopologyEditor.prototype.init = function (backImg,templateId,topologyId,stageJson,templateName) {
    ...
    this.scene.getDisplayedNodes().forEach(function (i) {
        editor.utils.updateLister(i);
        // var Location = editor.utils.valLocation(i);
        // i.setLocation(Location.x , Location.y );
        // i.mouseup(function (e) {
        //     var Location = editor.utils.valLocation(e.target);
    ...
    drawArea.ondrop = function (e) {
        ...
        if (data) {
            ...
            if (datas && datas.length == 3) {
                ...
                var local = editor.utils.valLocation({x:(e.layerX ? e.layerX : e.offsetX) - self.scene.translateX - self.config.defaultWidth / 2,y: (e.layerY ? e.layerY : e.offsetY) - self.scene.translateY - self.config.defaultHeight / 2})
                // node.setBound((e.layerX ? e.layerX : e.offsetX) - self.scene.translateX - self.config.defaultWidth / 2,
                //     (e.layerY ? e.layerY : e.offsetY) - self.scene.translateY - self.config.defaultHeight / 2,
                //     self.config.defaultWidth,self.config.defaultHeight);
                node.setBound(local.x,local.y,self.config.defaultWidth,self.config.defaultHeight);
                //设备图片
                node.setImage(context + '/js/roomlayout/icon/' + img);
                ...
                //子网连接点的下一个层,默认为0
                node.nextLevel = "0";
                editor.utils.updateLister(node);
 ...
 *
 * @param n
 * @param isAddToScene 是否添加到场景
 * @returns {*}
 */
networkTopologyEditor.prototype.addNode = function (n,isAddToScene) {
    var self = this;
    var newNode = null;
    if (n instanceof JTopo.Node||n instanceof JTopo.Container) {//
        newNode = n
    } else {
        ...
        newNode.setSize( n.width, n.height);
        newNode.text = n.text;
        newNode.setImage("icon/"+n.nodeImage);
        newNode.setLocation(n.x , n.y );
    }
    var a = n,c=newNode;
    for (var e in a) {
        //加载节点的图片
        if (e == "nodeImage") {
            c.setImage(context + '/js/roomlayout/icon/' + a[e]);
        }
        if (c) {
            if (a[e] == "undefined") {
                c[e] = undefined;
    ...
    editor.utils.updateLister(newNode);
    if(typeof (isAddToScene)== 'undefined'||isAddToScene){
        self.scene.add(newNode);
    }
    return newNode;
}
networkTopologyEditor.prototype.addMNode = function (n,c,z) {
    var self = this;
    for(var i= 0 ;i<c;i++){
        var newn = editor.utils.cloneObj(n);
        var newNode =  this.addNode(newn);
        var bx =newNode.x,by=newNode.y;
        if (z === 0)
            newNode.setLocation(bx + i * this.config.floorWidth, by);
        else
            newNode.setLocation(bx, by + i * this.config.floorHeight);
    }
}
...
editor.utils = {
    ...
    deleteSelectedNodes : function(){
        ...
        if(nodes && nodes.length > 0){
            jConfirm("确定要移除该位置布局吗?","移除布局",function(ok){
                if(ok){
    ...
    updateLister:function (node) {
        var Location = editor.utils.valLocation(node);
    ...
    valLocation :function (node) {
         var cx=node.x+editor.config.floorWidth/2 ,cy=node.y+editor.config.floorHeight/2;//+30;
         var x=(parseInt(cx/editor.config.floorWidth))*editor.config.floorWidth,
             y=(parseInt(cy/editor.config.floorHeight))*editor.config.floorHeight;
        return {x:x,y:y};
    },
