...
//            }
    ...
    public static List<Object[]> getTimes(String s, String e) {
        ...
        if(syear==eyear){
            ...
            if (smonth == emonth) {
                time.add(new Object[]{DateFormatUtils.format(sc,"yyyy_MM"), DateFormatUtils.format(sc.getTime(), "yyyy-MM-dd HH:mm:ss"), DateFormatUtils.format(ec.getTime(), "yyyy-MM-dd HH:mm:ss")});
            } else {
                Object[] ts = new Object[3];
                ts[0] = DateFormatUtils.format(sc, "yyyy_MM");
                ts[1] = DateFormatUtils.format(sc.getTime(), "yyyy-MM-dd HH:mm:ss");
            ...
            ec.set(Calendar.MONTH,0);
            ec.set(Calendar.HOUR_OF_DAY,0);
            ts[1] = DateFormatUtils.format(ec.getTime(), "yyyy-MM-dd HH:mm:ss");
            time.add(ts);
        }
...
//        Calendar c = Calendar.getInstance();
//        for (int i = 0; i < netCapPorts.size(); i++) {
//            NetCapPort value = netCapPorts.get(i);
//            Timestamp recordTime = value.getRecordTime();
//            c.setTime(recordTime);
//            //5分种向下取
//            double v = Math.floor(c.get(Calendar.MINUTE) / 5) * 5;
//            c.set(Calendar.MINUTE, (int) v);
...
//        //最后一个 如果时间超了后面的不补仓
//        netCapPorts.sort(new Comparator<NetCapPort>() {
//            @Override
...
//            }
    ...
    public static double getflow(double inflow, Double nextflow, int i1) {
