...
//      config.setMaxActive(p.getPropertyInt("redis.pool.maxActive"));
//      //最大空闲数：空闲链接数大于maxIdle时，将进行回收
//      config.setMaxIdle(p.getPropertyInt("redis.pool.maxIdle"));
//      //最小空闲数：低于minIdle时，将创建新的链接
//      config.setMinIdle(p.getPropertyInt("redis.pool.minIdle"));
//      //最大等待时间：单位ms
//      config.setMaxWait(p.getPropertyInt("redis.pool.maxWait"));
//      //使用连接时，检测连接是否成功
//      config.setTestOnBorrow(p.getPropertyBoolean("redis.pool.testOnBorrow"));
//      //返回连接时，检测连接是否成功
//      config.setTestOnReturn(p.getPropertyBoolean("redis.pool.testOnReturn"));
//      jp = new JedisPool(config, p.getProperty("redis.host"),
//          p.getPropertyInt("redis.port"), p.getPropertyInt("redis.timeout"));
      ApplicationContext c=new ClassPathXmlApplicationContext("spring-jedis.xml");
      shardedJedisPool=(ShardedJedisPool) c.getBean("shardedjedispool");
    }
    
    /**
     * 执行器，{@link RedisHelper}的辅助类，
    ...
    abstract class LockExecutor<T> extends Executor<T>{

        private String key;

        public LockExecutor(String key,ShardedJedisPool shardedJedisPool) {
...
//                }
     ...
     * @return 设置成功，返回 1 。设置失败，返回 0 。
     */
    public Long setStringIfNotExists(final String key,final int second, final String value) {
        return new Executor<Long>(shardedJedisPool) {

            @Override
            Long execute() {
                if (1l==jedis.setnx(key, value)){
    ...
    public Long listRemove(final String key, final long count, final String value){
        return new Executor<Long>(shardedJedisPool){
     ...
     * @return
     */
    public String set(final String key, final byte[] bytes) {
        return new Executor<String>(shardedJedisPool) {

            @Override
            String execute() {
                return jedis.set(key.getBytes(default_charset), bytes);
            }
        }.getResult();
    }
    ...
    public static byte[] getBytes(Object obj){
      ...
      public static Object getObject(byte[] bts){
