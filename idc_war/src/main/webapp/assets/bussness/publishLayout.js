...
function getRequestUrl(){
	return "/createPublishResource/listPage.do";
}
...
function menuEdit(key) {
	...
	layer.open({
		...
		maxmin : true,
		area: ['850px', '550px'],
		content: contextPath+"/createPublishResource/menuEdit.do?id="+record['id'],//iframe的url:  控制查看是否是编辑还是查看
		/*弹出框*/
...
function menuSearch(key) {
	...
	layer.open({
	      ...
	      maxmin : true,
		  area: ['850px', '550px'],
		  content: contextPath+"/createPublishResource/menuSearch.do?id="+record['id'],//iframe的url:  控制查看是否是编辑还是查看
		  /*弹出框*/
		  cancel: function(index, layero){ 
		    //右上角关闭回调
		    layer.close(index);
...
function menuSave(key) {
	var record = $('#gridId').datagrid("getRows")[key];
	//直接ajax进行提交
	layer.confirm('确定要提交该流程吗?', function(){ 
		//ajax
		$.ajax({
	        //提交数据的类型 POST GET
	        type:"POST",
