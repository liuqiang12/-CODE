...
<div style="width:1012px;height:30px;line-height:30px;background-color: #d7f5fe">
    <marquee  direction="left" scrollamount="30" scrolldelay="500" behavior="SCROLL" hspace="0" vspace="0" loop="INFINITE">
        <span style="color: #006107">订单是根据客户需求,由客户经理发起订单业务申请,经过审批,资源满足客户需求完成订单工单信息...</span>
    </marquee>
</div>
<div id='page'>
    <div id='menu'>
        <img src='<%=request.getContextPath() %>/framework/jqDock/icons/rack.png' title='' alt='' id="rack_img" onclick="addServiceItem(this,'100','机架机位');" />
        <img src='<%=request.getContextPath() %>/framework/jqDock/icons/port.png' title='' alt='' id="port_img" onclick="addServiceItem(this,'200','端口带宽');" />
        <img src='<%=request.getContextPath() %>/framework/jqDock/icons/IP.png' title='' alt='' id="ip_img" onclick="addServiceItem(this,'300','IP租用');" />
        <img src='<%=request.getContextPath() %>/framework/jqDock/icons/server.png' title='' alt='' id="server_img" onclick="addServiceItem(this,'400','主机租赁');" />
        <img src='<%=request.getContextPath() %>/framework/jqDock/icons/add.png' title='' alt='' id="add_img" onclick="addServiceItem(this,'500','增加业务');" />
        <!-- 如果是修改则需要显示客户信息 -->
        <c:if test="${not empty idcReProduct.id and idcReProduct.isActive == 1}">
            <img src='<%=request.getContextPath() %>/framework/jqDock/icons/activ_to_disabled.png' title='' alt='' onclick="ajaxProductFun(this,'${idcReProduct.id}','${idcReProduct.isActive}')"  />
        </c:if>
        <c:if test="${not empty idcReProduct.id and idcReProduct.isActive == 0}">
            <img src='<%=request.getContextPath() %>/framework/jqDock/icons/disabled-act.png' title='' alt='' onclick="ajaxProductFun(this,'${idcReProduct.id}','${idcReProduct.isActive}')"  />
        </c:if>
        <c:if test="${not empty idcReProduct.id}">
        ...
        <table class="kv-table">
            <tr>
                ...
                <td class="kv-content">
                    ...
                    <input class="easyui-textbox" data-options="required:true" value="${idcReProduct.prodName}" name="prodName" id="prodName" data-options="validType:'length[0,255]',width:150"/>
                </td>
                ...
                <td class="kv-content">
                    ...
                    <input id="customerName" value="${idcReProduct.customerName}" class="easyui-textbox" data-options="editable:false,required:true,iconAlign:'left',buttonText:'选择',onClickButton:openWinCustomerNameFun">
                </td>
            </tr>
        </table>
    </fieldset>
...
<script type="application/javascript">
    ...
    function ajaxProductFun(obj,id,actFlag){
        var actFlag_text = actFlag == 1?"激活转变成【禁用】":"禁用转变成【激活】";
        top.layer.confirm('你确定要将订单['+actFlag_text+']吗？', {
            btn: ['确定','取消'] //按钮
            ...
            $.ajax({
                ...
                type:"POST",
                //提交的网址
                url:contextPath+"/resourceApplyController/updateIsActive/"+id+"/"+actFlag,
                //提交的数据
                data:{
                    name: actFlag_text
                },
                ...
                success:function(data){
                    ...
                    if(actFlag == 1){
                        $(obj).attr("src",contextPath+"/framework/jqDock/icons/disabled-act.png")
                    }else{
                        $(obj).attr("src",contextPath+"/framework/jqDock/icons/activ_to_disabled.png")
                    }
                },
                //调用执行后调用的函数
    ...
    function form_sbmt(grid,jbpmFunFlag){
        ...
        $("#singleForm").form('submit', {
            url:contextPath+"/resourceApplyController/productFormSaveOrUpdate.do",
            onSubmit: function(){
        ...
        if (category == '100'){
            ...
            if($(".fieldsetContent").find("#rack_wrap") && $(".fieldsetContent").find("#rack_wrap").length == 0){
                ...
                var rackHTML =
                            ...
                            '   data-options=\"valueField: \'value\',textField: \'label\',editable:false,required:true,width:580,data: [{label: \'开放式机架\',value: \'1\'},{label: \'-------\',value: \'0\'}]" name="idcReRackModel.spec" id="idcReRackModel_spec" value="${idcReRackModel.SPEC}" />'+
                            '                </li>'+
                             '               <li>'+
                              '                  额定功率:<input class="easyui-numberbox" name="idcReRackModel.ratedPower" value="${idcReRackModel.RATEDPOWER}"'+
                               '                 id="idcReRackModel_ratedPower" data-options="width:580,precision:0,suffix:\'千瓦(KW)\',min:0,validType:\'length[0,12]\'"/>'+
                                            '</li>'+
                                           ' <li>'+
                                          '      &nbsp;&nbsp;&nbsp;机架数:<input class="easyui-numberbox" name="idcReRackModel.rackNum" value="${idcReRackModel.RACKNUM}"'+
                                         '       id="idcReRackModel_rackNum" data-options="width:580,precision:0,suffix:\'个\',min:0,validType:\'length[0,11]\'"/>'+
                                        '    </li>'+
                                       '     <li>'+
                                      '          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;描述:<input class="easyui-textbox" value="${idcReRackModel.DESC}"'+
            ...
            if($(".fieldsetContent").find("#port_wrap") && $(".fieldsetContent").find("#port_wrap").length == 0){
                ...
                '                   <ul>'+
                '                   <li>'+
                '                   带宽模式:<input class="easyui-combobox" data-options="'+
                '               valueField: \'value\','+
                '                   textField: \'label\','+
                '                   editable:false,'+
                '                   required:true,'+
                ...
                '               </li>'+
                '               <li>'+
                '               带宽大小(兆):<input class="easyui-numberbox" data-options="required:true,validType:\'length[0,11]\'" name="idcRePortModel.bandwidth" id="idcRePortModel_bandwidth" value="${idcRePortModel.bandwidth}" />'+
                '               </li>'+
                '               <li>'+
                '               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;数量:<input class="easyui-numberbox"'+
                '               name="idcRePortModel.num"'+
                '               value="${idcRePortModel.NUM}"'+
                '               id="idcReRackModel_num"'+
                '               data-options="width:580,precision:0,min:0,validType:\'length[0,11]\'"/>'+
                '                   </li>'+
                '                   <li>'+
                '                   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;描述:<input class="easyui-textbox" value="${idcRePortModel.DESC}"'+
            ...
            if($(".fieldsetContent").find("#ip_wrap") && $(".fieldsetContent").find("#ip_wrap").length == 0){
                ...
                '               </li>'+
                '               <li>'+
                '               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;描述:<input class="easyui-textbox" value="${idcReIpModel.DESC}"'+
            ...
            if($(".fieldsetContent").find("#zjserver_wrap") && $(".fieldsetContent").find("#zjserver_wrap").length == 0){
                ...
                '                   <ul>'+
                '                   <li>'+
                '                   资源类型:<input class="easyui-combobox" data-options="'+
                '               valueField: \'value\','+
                '                   textField: \'label\','+
                '                   editable:false,'+
                '                   required:true,'+
                ...
                '               </li>'+
                '               <li>'+
                '               设备型号:<input class="easyui-textbox" value="${idcReServerModel.specNumber}"'+
                '               name="idcReServerModel.specNumber" id="idcReServerModel_specNumber"'+
                '               data-options="width:580"/>'+
                '                   </li>'+
                '                   <li>'+
                '                   设备编码:<input class="easyui-textbox" value="${idcReServerModel.code}"'+
                '               name="idcReServerModel.code" id="idcReServerModel_code"'+
                '               data-options="width:580"/>'+
                '                   </li>'+
                '                   <li>'+
                '                   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;品牌:<input class="easyui-textbox" value="${idcReServerModel.brand}"'+
                '               name="idcReServerModel.brand" id="idcReServerModel_brand"'+
                '               data-options="width:580"/>'+
                '                   </li>'+
                '                   <li>'+
                '                   额定功率:<input class="easyui-numberbox" name="idcReServerModel.ratedPower" value="${idcReServerModel.ratedPower}"'+
                ...
                '                   </li>'+
                '                   <li>'+
                '                   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;描述:<input class="easyui-textbox" value="${idcReServerModel.desc}"'+
            ...
            if($(".fieldsetContent").find("#add_wrap") && $(".fieldsetContent").find("#add_wrap").length == 0){
                ...
                '                       <ul>'+
                '                       <li>'+
                '                       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名称:<input class="easyui-textbox" value="${idcReNewlyModel.NAME}"'+
                '                   name="idcReNewlyModel.name" id="idcReNewlyModel_name"'+
                '                   data-options="width:580"/>'+
                '                       </li>'+
                '                       <li>'+
                '                       业务分类:<input class="easyui-combobox" data-options="'+
                '                   valueField: \'value\','+
                '                       textField: \'label\','+
                '                       editable:false,'+
                '                       width:580,'+
                ...
                '                       label: \'----\','+
                '                       value: \'2\''+
                '                   }]" name="idcReNewlyModel.category" id="idcReNewlyModel_category" value="${idcReNewlyModel.CATEGORY}" />'+
                '                   </li>'+
                '                   <li>'+
                '                   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;描述:<input class="easyui-textbox" value="${idcReNewlyModel.DESC}"'+
